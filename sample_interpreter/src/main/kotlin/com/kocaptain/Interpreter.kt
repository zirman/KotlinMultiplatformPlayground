...
fun <A, B, C> evaluate(free: Free3<A, B, C>, cont: (C) -> Unit): Unit {
    @Suppress("UNUSED_VARIABLE") // unused variable forces match on all operations
    val x: Unit =
        when (free) {
            ...
            is Zip<A, B, C> -> {
                Single
                    .zip<A, B, Free1<C>>(
                        Single.create { emitter -> evaluate(free.free1) { emitter.onSuccess(it) } },
                        Single.create { emitter -> evaluate(free.free2) { emitter.onSuccess(it) } },
    ...
    val x: Unit =
        when (s) {
            ...
            is GetRequestString -> {
                ...
                s.unerase(cont).invoke(Either.Left(Error("GetRequestString not implemented yet")))
                Unit
            }
